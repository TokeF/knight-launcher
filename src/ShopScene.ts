...
export class ShopScene extends Phaser.Scene {
  ...
  create() {
    this.add.text(this.cameras.main.width / 2, 50, 'Shop', { fontSize: '48px', color: '#fff' }).setOrigin(0.5);

    let yPos = 120;
    this.items.forEach((item) => {
      this.add.image(100, yPos, item.imageKey).setOrigin(0, 0.5);
      this.add.text(200, yPos - 20, item.stats, { fontSize: '24px', color: '#0f0' });
      this.add.text(200, yPos + 10, item.description, { fontSize: '18px', color: '#fff', wordWrap: { width: 400 } });
      yPos += 90;
    });

    const backButton = this.add
      .text(this.cameras.main.width - 100, this.cameras.main.height - 50, 'Back', {
        fontSize: '32px',
        color: '#ff0',
      })
      .setOrigin(0.5)
